...
<p align="center">
  <code>71 Actions</code> &nbsp;&middot;&nbsp; <code>29 Context Prompts</code> &nbsp;&middot;&nbsp; <code>276 Native C++ Functions</code> &nbsp;&middot;&nbsp; <code>~27,100 Lines of Papyrus</code>
</p>
...
| **OutfitDataStore** | Native cosave persistence for outfit locks and presets — locked item lists and named outfit presets per actor. Survives save/load and dismissal/re-recruit cycles |
| **PrismaUI Bridge** | Bidirectional C++ ↔ React communication layer for the in-game config menu — data gathering, setting writes, and action dispatch all handled natively for sub-20ms response times |
...
| **Companions** | Visual party roster with relationship bars (rapport/trust/loyalty/mood), combat style selection, home assignment, teleport-to-player, dismiss, package management |
| **World** | Per-hold bounty grid with clear buttons, active travel slots, currency settings, debt summary |
| **Settings** | Dialogue animations, silence chance, speaker tags, survival needs (hunger/fatigue/cold rates), book reading mode, spell failure settings, UI scale |
...
- **Automatic relationship assessment** — Background LLM evaluation of follower relationships every 5 game hours. The AI reviews recent events and memories, then adjusts rapport, trust, loyalty, and mood organically — no action slot needed
...
- **Persistent outfit locking** — Locked outfits survive cell transitions, save/load, dismissal, and engine auto-equip. Uses per-follower ReferenceAlias slots with `OnLoad`/`OnCellLoad`/`OnObjectUnequipped` events for zero-flicker re-equipping
