...
Event OnPrismaInventorySync(String eventName, String strArg, Float numArg, Form sender)
    ...
    ; C++ already removed the item from OutfitDataStore.
    ; Rebuild StorageUtil FormList from what's actually still equipped.
    String lockKey = "SeverOutfit_Locked_" + (akActor.GetFormID() as String)
    ...
    ; Clear and rebuild from worn items
    StorageUtil.FormListClear(None, lockKey)
    Int slot = 0x00000001
    While slot < 0x80000000
        Form wornForm = akActor.GetWornForm(slot)
        If wornForm && StorageUtil.FormListFind(None, lockKey, wornForm) < 0
            StorageUtil.FormListAdd(None, lockKey, wornForm)
        EndIf
        slot = Math.LeftShift(slot, 1)
    EndWhile
