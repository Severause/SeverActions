...
Event OnPlayerLoadGame()
    ...
    ; Register for PrismaUI travel actions (ModEvents — DispatchMethodCall silently fails)
        ...
        If theAlias
            ...
            If npc && !npc.IsDead()
                ...
                If npcState == "traveling"
                    ...
                    Int speed = StorageUtil.GetIntValue(npc, "SeverTravel_Speed", 0)
                    SlotSpeeds[i] = speed
                    ; Re-apply PO3 travel package override (lost on save/load)
                    ; LinkedRef persists via native cosave, so the package knows where to go
                    Package travelPkg = GetTravelPackageForSpeed(speed)
                    If travelPkg
                        ActorUtil.AddPackageOverride(npc, travelPkg, TravelPackagePriority, 1)
                        npc.EvaluatePackage()
...
Bool Function TravelToReference(Actor akNPC, ObjectReference akDestination, Float waitHours = 0.0, Bool stopFollowing = false, Int speed = 1, Bool externalManaged = false)
    ...
    SeverActionsNative.Native_SetTravelState(akNPC, "traveling", "dispatch_target")
    StorageUtil.SetFloatValue(akNPC, "SeverTravel_WaitUntil", waitUntil)
    StorageUtil.SetIntValue(akNPC, "SeverTravel_Slot", slot)
    StorageUtil.SetIntValue(akNPC, "SeverTravel_Speed", speed)
...
Function ClearSlot(Int slot, Bool restoreFollower = false)
    ...
    If theAlias
        ...
        If npc
            ...
            SeverActionsNative.Native_SetTravelState(npc, "", "")
