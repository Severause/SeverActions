...
; Fires "SeverActionsNative_OnArrival" when actor is within threshold of destination.
; One-shot: auto-unregisters after firing. Auto-cancels on actor death.
; 2-second grace period after registration to prevent false positives.
; =============================================================================
...
{Monitor actor distance to ObjectReference. Fires "SeverActionsNative_OnArrival" with strArg=callbackTag when within threshold. One actor = one registration (overwrites previous).}
...
{Monitor actor distance to XY coordinates. Same behavior as Arrival_Register.}

Function Arrival_Cancel(Actor akActor) Global Native
{Stop monitoring an actor. No event fires.}
...
{Get current distance to tracked destination. Returns -1 if not tracked or no position data available.}

Function Arrival_ClearAll() Global Native
{Clear all tracking. Called automatically on game load/new game.}
