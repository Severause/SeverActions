...
Function RegisterForPrismaEvents()
    ...
    If SeverActionsNative.PrismaUI_IsAvailable()
        ...
        RegisterForModEvent("SeverActions_PrismaUI_SettingChanged", "OnSettingChanged")
        RegisterForModEvent("SeverActions_PrismaUI_ActionRequested", "OnActionRequested")
        Debug.Trace("[SeverActions_PrismaUI] Registered for PrismaUI mod events")
    Else
...
Event OnSettingChanged(String eventName, String strArg, Float numArg, Form sender)
    ...
    If page == "settings"
        ; Settings consolidates: dialogue/tags (General), survival config, outfit config
        HandleGeneralSetting(sKey, sVal)
        HandleSurvivalSetting(sKey, sVal)
        HandleOutfitsSetting(sKey, sVal, sTarget)
    ElseIf page == "world"
    ...
    ElseIf page == "companions"
        ; Companions consolidates: follower framework, per-NPC outfits
        HandleFollowersSetting(sKey, sVal, sTarget)
        HandleOutfitsSetting(sKey, sVal, sTarget)
    ElseIf page == "hotkeys"
        HandleHotkeysSetting(sKey, sVal)
    EndIf
EndEvent
...
Function SendWorldData()
    ...
    String[] playerOwes = None
    String[] owedToPlayer = None
    If DebtScript
        playerOwes = DebtScript.GetPlayerOwesDetails()
        owedToPlayer = DebtScript.GetOwedToPlayerDetails()
    EndIf

    SeverActionsNative.PrismaUI_BeginArray("playerOwes")
    Int pi = 0
    If playerOwes
        While pi < playerOwes.Length
            SeverActionsNative.PrismaUI_PushString(playerOwes[pi])
            pi += 1
        EndWhile
    EndIf
    SeverActionsNative.PrismaUI_EndArray()
    ...
    Int oi = 0
    If owedToPlayer
        While oi < owedToPlayer.Length
            SeverActionsNative.PrismaUI_PushString(owedToPlayer[oi])
            oi += 1
        EndWhile
    EndIf
    SeverActionsNative.PrismaUI_EndArray()
...
Function SendHotkeysData()
    {Build hotkeys page: key bindings + targeting settings.
     Papyrus fallback when C++ ScriptReader can't read MCM properties.}
    Debug.Trace("[SeverActions_PrismaUI] SendHotkeysData START")
    SeverActionsNative.PrismaUI_BeginPage("hotkeys")

    If MCMScript
        ; Key bindings
        SeverActionsNative.PrismaUI_AddInt("wheelMenuKey", MCMScript.WheelMenuKey)
        SeverActionsNative.PrismaUI_AddInt("followToggleKey", MCMScript.FollowToggleKey)
        SeverActionsNative.PrismaUI_AddInt("dismissKey", MCMScript.DismissKey)
        SeverActionsNative.PrismaUI_AddInt("setCompanionKey", MCMScript.SetCompanionKey)
        SeverActionsNative.PrismaUI_AddInt("companionWaitKey", MCMScript.CompanionWaitKey)
        SeverActionsNative.PrismaUI_AddInt("assignHomeKey", MCMScript.AssignHomeKey)
        SeverActionsNative.PrismaUI_AddInt("standUpKey", MCMScript.StandUpKey)
        SeverActionsNative.PrismaUI_AddInt("yieldKey", MCMScript.YieldKey)
        SeverActionsNative.PrismaUI_AddInt("undressKey", MCMScript.UndressKey)
        SeverActionsNative.PrismaUI_AddInt("dressKey", MCMScript.DressKey)

        ; Targeting
        SeverActionsNative.PrismaUI_AddInt("targetMode", MCMScript.TargetMode)
        SeverActionsNative.PrismaUI_AddFloat("nearestNPCRadius", MCMScript.NearestNPCRadius)

        ; Status
        If MCMScript.WheelMenuKey > 0
            SeverActionsNative.PrismaUI_AddString("wheelMenuStatus", "Active")
        Else
            SeverActionsNative.PrismaUI_AddString("wheelMenuStatus", "Not Set")
        EndIf
    EndIf

    ; Hotkey system status
    If HotkeyScript
        SeverActionsNative.PrismaUI_AddString("hotkeyStatus", "Active")
    Else
        SeverActionsNative.PrismaUI_AddString("hotkeyStatus", "Inactive")
    EndIf

    SeverActionsNative.PrismaUI_SendPage()
    Debug.Trace("[SeverActions_PrismaUI] SendHotkeysData DONE")
EndFunction

Function SendSettingsData()
    {Build settings page: dialogue/tags, survival, outfit, spell teach config.
     Papyrus fallback when C++ ScriptReader can't read MCM properties.}
    ...
    SeverActionsNative.PrismaUI_AddBool("hasOutfitScript", OutfitScript != None)
    If OutfitScript
        SeverActionsNative.PrismaUI_AddBool("useAnimations", OutfitScript.UseAnimations)
        SeverActionsNative.PrismaUI_AddBool("outfitLockEnabled", OutfitScript.OutfitLockEnabled)
    EndIf
...
Function HandleOutfitsSetting(String sKey, String sVal, String sTarget)
    If !OutfitScript
        Return
    EndIf
    If sKey == "useAnimations"
        OutfitScript.UseAnimations = (sVal == "true")
    ElseIf sKey == "outfitLockEnabled"
        OutfitScript.OutfitLockEnabled = (sVal == "true")
    ElseIf sKey == "outfitLocked" && sTarget != ""
        Actor npc = FindOutfitNPCByName(sTarget)
        If npc
            If sVal == "true"
                OutfitScript.SnapshotLockedOutfit(npc)
            Else
                OutfitScript.ClearLockedOutfit(npc)
            EndIf
        EndIf
    EndIf
EndFunction
...
Function HandleAction(String actType, String json)
    ...
    ; Outfit actions
    ElseIf actType == "snapshotOutfit"
        actName = SeverActionsNative.PrismaUI_ExtractJsonValue(json, "name")
        actTarget = FindOutfitNPCByName(actName)
        If actTarget && OutfitScript
            OutfitScript.SnapshotLockedOutfit(actTarget)
        EndIf
        SeverActionsNative.PrismaUI_RefreshPage("companions")
    ElseIf actType == "clearOutfitLock"
        actName = SeverActionsNative.PrismaUI_ExtractJsonValue(json, "name")
        actTarget = FindOutfitNPCByName(actName)
        If actTarget && OutfitScript
            OutfitScript.ClearLockedOutfit(actTarget)
        EndIf
        SeverActionsNative.PrismaUI_RefreshPage("companions")
    ElseIf actType == "applyPreset"
        actName = SeverActionsNative.PrismaUI_ExtractJsonValue(json, "npc")
        actLocName = SeverActionsNative.PrismaUI_ExtractJsonValue(json, "preset")
        actTarget = FindOutfitNPCByName(actName)
        If actTarget && OutfitScript
            OutfitScript.ApplyOutfitPreset_Execute(actTarget, actLocName)
        EndIf
        Utility.Wait(1.0)
        SeverActionsNative.PrismaUI_RefreshPage("companions")
    ElseIf actType == "deletePreset"
        actName = SeverActionsNative.PrismaUI_ExtractJsonValue(json, "npc")
        actLocName = SeverActionsNative.PrismaUI_ExtractJsonValue(json, "preset")
        actTarget = FindOutfitNPCByName(actName)
        If actTarget && OutfitScript
            OutfitScript.DeletePreset(actTarget, actLocName)
        EndIf
        SeverActionsNative.PrismaUI_RefreshPage("companions")
...
Actor Function FindOutfitNPCByName(String name)
    {Find an NPC from outfit-locked or preset actor lists by display name.}
    If !OutfitScript || name == ""
        Return None
    EndIf
    ; Search locked actors first
    Actor[] locked = OutfitScript.GetOutfitLockedActors()
    Int i = 0
    If locked
        While i < locked.Length
            If locked[i] && locked[i].GetDisplayName() == name
                Return locked[i]
            EndIf
            i += 1
        EndWhile
    EndIf
    ; Then search preset actors
    Actor[] presets = OutfitScript.GetPresetActors()
    i = 0
    If presets
        While i < presets.Length
            If presets[i] && presets[i].GetDisplayName() == name
                Return presets[i]
            EndIf
            i += 1
        EndWhile
    EndIf
    ; Fall back to follower search
    Return FindFollowerByName(name)
EndFunction
