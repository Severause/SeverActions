...
Function RegisterForPrismaEvents()
    ...
    If SeverActionsNative.PrismaUI_IsAvailable()
        ...
        Debug.Trace("[SeverActions_PrismaUI] Registered for PrismaUI data request event (settings/actions now C++)")
    Else
...
Function HandleClearAllPackages(Actor akActor)
    {Called by C++ PrismaUIActionHandler for "clearAllPackages" action.}
    If akActor
...
Function HandleRemovePackage(Actor akActor, String packageName)
    {Called by C++ PrismaUIActionHandler for "removePackage" action.}
    If akActor
...
Function ResetAllCompanions()
    {Called by C++ PrismaUIActionHandler for "resetAllCompanions" action.}
    If FollowerManagerScript
        Actor[] allComp = FollowerManagerScript.GetAllFollowers()
        If allComp
            Int ci = 0
