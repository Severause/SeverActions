...
Function UnregisterFollower(Actor akActor, Bool sendHome = true)
    ...
    Else
        ...
        If followSys
            followSys.CompanionStopFollowing(akActor)
        EndIf
        ; Do NOT call SendHome — the framework handles where the NPC goes.
    EndIf
...
Function AssignHome(Actor akActor, String locationName)
    ...
    ; Try to resolve a marker for the location (optional — SendHome resolves on its own)
    ...
    Else
        ...
        DebugMsg("No marker resolved for home '" + locationName + "' — SendHome will resolve at travel time")
    EndIf
...
Function SendHome(Actor akActor)
    {Send a follower to their assigned home. If no home is assigned, do nothing —
     CompanionStopFollowing already cleared all overrides so the NPC returns to
     their default AI (vanilla packages, editor routines, etc.).}
    If !akActor
        Return
    EndIf
    ...
    ; No home assigned — NPC returns to default AI naturally.
    ; CompanionStopFollowing already removed all package overrides and called
    ; EvaluatePackage, so the engine will pick up the NPC's vanilla package stack.
    DebugMsg(akActor.GetDisplayName() + " dismissed with no home — returning to default AI")
EndFunction
