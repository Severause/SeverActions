...
Int Property FrameworkMode = 2 Auto
...
Event OnNativeTeammateDetected(string eventName, string strArg, float numArg, Form sender)
    ...
    ; Custom AI followers (SPID keyword) are always allowed through — they're
    ; known modded followers we want to track for relationships, PrismaUI, and
    ; outfit lock, even though they don't join vanilla follower factions.
...
Bool Function IsTrackOnlyFollower(Actor akActor)
    {Returns true if this actor should be tracked but not package-managed.
     ...
     DLC-managed followers (Serana), and Tracking Only mode (FrameworkMode == 2).}
    If FrameworkMode == 2
        Return true
    EndIf
...
Bool Function HasCustomAIKeyword(Actor akActor)
    ...
    Keyword customAIKW = Game.GetFormFromFile(0x13258A, "SeverActions.esp") as Keyword
    If !customAIKW
        Return false
    EndIf
...
Function RegisterFollower(Actor akActor)
    ...
    ; Tracking Only mode: if the actor is ALREADY a teammate (managed by another mod),
    ; treat them as track-only — don't touch their AI, factions, or follow packages.
    ; But if they're NOT already a teammate (e.g. recruited via SetCompanion hotkey),
    ; they need full SeverActions setup (SetPlayerTeammate, factions, packages) or
    ; the engine won't recognize them as a follower (no inventory, no idles, no dialogue).
    If FrameworkMode == 2 && wasAlreadyTeammate
        wasAlreadyTeammate = true  ; redundant but explicit — track-only path
    EndIf

    ; Determine routing: framework vs our system vs track-only
    ; In Tracking Only mode, actors who were NOT already teammates need full setup
    ; (SetPlayerTeammate, factions, dialogue alias) or the engine won't recognize them.
    ; Only truly pre-existing teammates should be track-only.
    Bool useFramework = ShouldUseFramework(akActor)
    Bool isTrackOnly = IsTrackOnlyFollower(akActor)
    Bool wantsTrackOnly = isTrackOnly  ; Preserve original intent for package skip
    If isTrackOnly && !wasAlreadyTeammate
        isTrackOnly = false
        DebugMsg(akActor.GetDisplayName() + " needs engine setup (not teammate) but will skip our packages (track-only intent)")
    EndIf
    ...
    ElseIf IsSerana(akActor) && !wasAlreadyTeammate
        ; Serana DLC routing: use her mental model quest's EngageFollowBehavior()
        ; This sets her custom flags, calls SetPlayerTeammate(), adds to
        ; WIFollowerCommentFaction, and starts her monitoring quest — identical
        ; to what the "Come with me" dialogue does.
        ...
        Else
            ; Fallback — DLC quest not ready (quest hasn't progressed far enough)
            DebugMsg("Serana DLC routing FAILED — quest not ready, using SeverActions path")
            akActor.SetPlayerTeammate(true)
            akActor.IgnoreFriendlyHits(true)
        EndIf
    ElseIf wasAlreadyTeammate || isTrackOnly
        ; Track-only: custom framework follower, NFF-ignore-token holder, or other DLC-managed
    ...
    Else
        ; Vanilla/SeverActions path: route through vanilla DialogueFollower.SetFollower()
        ; This replicates the exact "Follow me" dialogue behavior: removes from
        ; DismissedFollower faction, sets rank >= 3, calls SetPlayerTeammate(),
        ; forces into pFollowerAlias, sets PlayerFollowerCount.
        DebugMsg("Vanilla routing: " + akActor.GetDisplayName())
        ...
        ; Try vanilla DialogueFollower.SetFollower() first — gives them full vanilla dialogue
        Bool vanillaRecruited = RecruitViaVanillaDialogue(akActor)
        If vanillaRecruited
            StorageUtil.SetIntValue(akActor, "SeverActions_RecruitedViaVanilla", 1)
            akActor.IgnoreFriendlyHits(true)
            DebugMsg("Vanilla SetFollower succeeded for " + akActor.GetDisplayName())
        Else
            ; Fallback: manual setup (vanilla quest unavailable)
            DebugMsg("Vanilla SetFollower failed — manual setup for " + akActor.GetDisplayName())
            akActor.SetPlayerTeammate(true)
            akActor.IgnoreFriendlyHits(true)
            Faction currentFollowerFaction = Game.GetFormFromFile(0x0005C84E, "Skyrim.esm") as Faction
            If currentFollowerFaction
        ...
        ; If NFF is in the load order and we're in SeverActions Only mode, give the actor
        ; an NFF ignore token so NFF doesn't try to manage them behind our back.
        ; NFF monitors CurrentFollowerFaction — the token tells it to leave this NPC alone.
        ; We track this with a StorageUtil flag so we only remove tokens WE added on dismiss.
        If HasNFF() && FrameworkMode == 1
            Form ignoreToken = Game.GetFormFromFile(0x051CFC8D, "nwsFollowerFramework.esp")
            If ignoreToken && akActor.GetItemCount(ignoreToken) == 0
                akActor.AddItem(ignoreToken, 1, true)
                StorageUtil.SetIntValue(akActor, "SeverActions_AddedNFFToken", 1)
                DebugMsg("Added NFF ignore token to " + akActor.GetDisplayName() + " (SeverActions Only mode)")
            EndIf
        EndIf
    EndIf
    ...
    ; Ensure relationship rank >= 3 (Ally) for ALL followers regardless of routing.
    ; Vanilla follower dialogue ("I need you to do something" / inventory access)
    ; requires relationship rank 3+. Track-only and framework-routed followers
    ; may not have this set. Save original rank for restoration on dismiss.
    Actor playerRef = Game.GetPlayer()
    If akActor.GetRelationshipRank(playerRef) < 3
        If !StorageUtil.HasIntValue(akActor, KEY_ORIG_RELRANK)
            StorageUtil.SetIntValue(akActor, KEY_ORIG_RELRANK, akActor.GetRelationshipRank(playerRef))
        EndIf
        akActor.SetRelationshipRank(playerRef, 3)
        DebugMsg("Set relationship rank to Ally for " + akActor.GetDisplayName())
    EndIf

    ; Ensure the vanilla DialogueFollower quest alias is filled so the engine
    ; recognizes this NPC for follower idle voice lines, "I need you to do something"
    ; inventory access, and other dialogue options. Without this, NPCs recruited via
    ; hotkey/wheel (bypassing vanilla dialogue) are teammates but lack dialogue features.
    ; Only fill if the alias is currently empty — don't clobber NFF/EFF/custom setups.
    ...
    If dialogueFollowerQuest
        ...
        If followerAlias
            ObjectReference currentRef = followerAlias.GetReference()
            If currentRef == None
                followerAlias.ForceRefTo(akActor)
                DebugMsg("Filled DialogueFollower alias for " + akActor.GetDisplayName())
            EndIf
        EndIf
    EndIf
    ...
    ; DLC-managed followers (Serana), ignore-token holders, and track-only intent.
    ; wantsTrackOnly preserves the original IsTrackOnlyFollower result even when
    ; isTrackOnly was cleared for engine setup (SetPlayerTeammate/factions).
    If !nffController && !effController && !wasAlreadyTeammate && !wantsTrackOnly
        SeverActions_Follow followSys = GetFollowScript()
        If followSys
            followSys.CompanionStartFollowing(akActor)
        EndIf
...
Function UnregisterFollower(Actor akActor, Bool sendHome = true)
    ...
    If nffController
        ...
        ; Safety net: NFF may silently fail for NPCs it never registered (mod followers,
        ; actors recruited via dialogue). Wait briefly then verify faction was actually removed.
        Utility.Wait(0.5)
        Faction currentFollowerFaction = Game.GetFormFromFile(0x0005C84E, "Skyrim.esm") as Faction
        If currentFollowerFaction && akActor.IsInFaction(currentFollowerFaction)
            DebugMsg("NFF dismiss safety net: " + akActor.GetDisplayName() + " still in CurrentFollowerFaction after NFF.RemoveFollower — clearing manually")
            akActor.RemoveFromFaction(currentFollowerFaction)
            akActor.SetPlayerTeammate(false)
            Faction playerFollowerFaction = Game.GetFormFromFile(0x084D1B, "Skyrim.esm") as Faction
            If playerFollowerFaction
    ...
    ElseIf effController
        ...
        StorageUtil.UnsetIntValue(akActor, KEY_WAS_ALREADY_TEAMMATE)
        frameworkHandledAI = true
    ElseIf StorageUtil.GetIntValue(akActor, "SeverActions_RecruitedViaSerana", 0) == 1
        ; Serana DLC dismiss: route through DisengageFollowBehavior()
        ; This clears her flags, calls SetPlayerTeammate(false), removes from
        ; WIFollowerCommentFaction, and stops her monitoring quest.
    ...
    ElseIf wasAlreadyTeammate && IsTrackOnlyFollower(akActor)
        ; Track-only cleanup: NFF-ignore-token holder, DLC-managed, or Tracking Only mode
        ; These actors were already teammates when recruited — we tracked them but didn't
        ; set up factions. However, if WE recruited them (via SeverActions dialogue/action),
        ; we DID add factions, so we must clean them up on dismiss.
        ; Remove factions and teammate status to ensure they actually stop following.
        DebugMsg("Track-only dismiss: " + akActor.GetDisplayName() + " — cleaning up factions and teammate status")
        akActor.SetPlayerTeammate(false)
        Faction currentFollowerFaction = Game.GetFormFromFile(0x0005C84E, "Skyrim.esm") as Faction
        If currentFollowerFaction && akActor.IsInFaction(currentFollowerFaction)
            akActor.RemoveFromFaction(currentFollowerFaction)
        EndIf
        Faction playerFollowerFaction = Game.GetFormFromFile(0x084D1B, "Skyrim.esm") as Faction
        If playerFollowerFaction && akActor.IsInFaction(playerFollowerFaction)
            akActor.RemoveFromFaction(playerFollowerFaction)
        EndIf
        StorageUtil.UnsetIntValue(akActor, KEY_WAS_ALREADY_TEAMMATE)
        frameworkHandledAI = true
    Else
        ...
        ; If we recruited via vanilla DialogueFollower, dismiss through it first
        Bool vanillaDismissed = false
        If StorageUtil.GetIntValue(akActor, "SeverActions_RecruitedViaVanilla", 0) == 1
            vanillaDismissed = DismissViaVanillaDialogue(akActor)
            StorageUtil.UnsetIntValue(akActor, "SeverActions_RecruitedViaVanilla")
            If vanillaDismissed
                DebugMsg("Vanilla DismissFollower succeeded for " + akActor.GetDisplayName())
            Else
                DebugMsg("Vanilla DismissFollower failed — manual cleanup for " + akActor.GetDisplayName())
            EndIf
        EndIf

        DebugMsg("SeverActions dismiss: " + akActor.GetDisplayName())

        ; If vanilla DismissFollower already ran, it handled SetPlayerTeammate(false),
        ; faction removal, and alias clearing. Only do manual cleanup if it didn't.
        If !vanillaDismissed
            akActor.SetPlayerTeammate(false)
        ...
        ; Remove NFF ignore token if WE added it during SeverActions Only recruitment.
        ; This allows NFF to manage the NPC again if the user switches back to Auto mode.
        If StorageUtil.GetIntValue(akActor, "SeverActions_AddedNFFToken", 0) == 1
            Form ignoreToken = Game.GetFormFromFile(0x051CFC8D, "nwsFollowerFramework.esp")
            If ignoreToken && akActor.GetItemCount(ignoreToken) > 0
                akActor.RemoveItem(ignoreToken, 1, true)
                DebugMsg("Removed NFF ignore token from " + akActor.GetDisplayName() + " (dismiss)")
            EndIf
            StorageUtil.UnsetIntValue(akActor, "SeverActions_AddedNFFToken")
        EndIf
...
Function OnOffScreenLifeEvent(String response, Int success)
    ...
    If involvedStr != ""
        ...
        While searchFrom < StringUtil.GetLength(involvedStr)
            String involvedName = ""
            If commaPos >= 0
                involvedName = StringUtil.Substring(involvedStr, searchFrom, commaPos - searchFrom)
                searchFrom = commaPos + 1
                commaPos = StringUtil.Find(involvedStr, ",", searchFrom)
            Else
                involvedName = StringUtil.Substring(involvedStr, searchFrom)
            ...
            If involvedName != ""
                ...
                If involvedActor && involvedActor != akActor
                    ...
                    EndIf
                    DebugMsg("Off-screen life: shared event registered for " + involvedName)
                EndIf
            EndIf
        EndWhile
    EndIf
